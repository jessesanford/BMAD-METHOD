...
# Git env vars inherited from a caller (e.g. a pre-commit hook) redirect every git
# invocation below into the caller's repository. Scrub them so the temp repos are hermetic.
for _name in ("GIT_INDEX_FILE", "GIT_DIR", "GIT_WORK_TREE", "GIT_OBJECT_DIRECTORY",
...
SUBMIT_PATH = SCRIPTS.parents[1] / "bmad-submit-prs" / "scripts" / "submit_pr_stack.py"
SUBMIT = module("submit", SUBMIT_PATH) if SUBMIT_PATH.exists() else None
...
class EvidenceTests(unittest.TestCase):
    ...
    @unittest.skipUnless(SUBMIT, "bmad-submit-prs is not installed")
    def test_two_prefix_evidence_satisfies_submit_report_contract(self) -> None:
        ...
        manifest = {"repository": "example/project", "_head_repository": "example/project", "publish_remote": "origin",
                    "evidence_remote": "origin", "_evidence_repository": "example/project",
