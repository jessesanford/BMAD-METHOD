...
class RebaseCascadeInvariantTests(unittest.TestCase):
    ...
    def test_pushes_only_stack_branches_to_origin(self) -> None:
        region = step(6)
        commands = re.findall(r"`git push [^`]+`", region)
        self.assertEqual(len(commands), 1, commands)
        push = " ".join(commands[0].split())
        self.assertRegex(push, r"--force-with-lease=refs/heads/")
        self.assertRegex(push, r"\borigin\b")
        self.assertNotRegex(push, r"\bupstream\b")
        self.assertRegex(
            " ".join(region.split()),
            r"Never publish\s+a source story/planning branch to `upstream`",
        )
        self.assertNotRegex(TEXT, r"git push[^`]*?(?:--mirror)")
